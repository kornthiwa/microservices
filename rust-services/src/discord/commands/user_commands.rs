...
async fn register_user(ctx: &Context, command: &CommandInteraction) -> serenity::Result<()> {
    ...
    let guild: serenity::all::PartialGuild = command.guild_id.unwrap().to_partial_guild(&ctx.http).await.unwrap();
    let guild_name: String = guild.name.clone();
    
    let member: serenity::all::Member = command.guild_id.unwrap().member(&ctx.http, command.user.id).await.unwrap();
    ...
    match UserService::check_user_exists(&user_id, &guild_id).await {
        ...
        Ok(false) => {
            match UserService::register_user(&guild_id, &user_id, &guild_name, &guild_user_nickname, &global_name, &user_name).await {
                Ok(_) => {
                    show_user_info_ui(
                        command,
                        ctx,
        ...
        Ok(Some(user)) => {
            let description = format!(
                ...
                **วันที่ลงทะเบียน:** {}",
                ...
                user.guild_name,
                chrono::DateTime::<chrono::Utc>::from(user.created_at.to_system_time()).format("%d/%m/%Y %H:%M:%S").to_string()
            );
...
async fn update_user(ctx: &Context, command: &CommandInteraction) -> serenity::Result<()> {
    ...
    let member: serenity::all::Member = command.guild_id.unwrap().member(&ctx.http, command.user.id).await.unwrap();
    let guild_user_nickname: String = member.nick.as_ref().unwrap_or(&member.user.name).to_string();
    let global_name: String = command.user.global_name.clone().unwrap_or_default();
    let user_name: String = command.user.name.clone();
                ...
                Ok(_) => {
                    let description = format!(
                        ...
                        **วันที่อัพเดท:** {}",
                        ...
                        user.guild_user_nickname,
                        user.guild_name,
                        chrono::DateTime::<chrono::Utc>::from(user.updated_at.to_system_time()).format("%d/%m/%Y %H:%M:%S").to_string()
                    );
